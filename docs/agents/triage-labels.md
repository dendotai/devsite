...
- `in-progress` — an agent is implementing this issue right now; applied by `/implement` when it starts. Not grabbable by `/next`, which prints it as a one-line status instead so a stalled one stays visible. The PR's `Closes #N` closes the issue on merge; the maintenance workflow then strips the label.
